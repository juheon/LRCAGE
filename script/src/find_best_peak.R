...
dfm.onepeak<-dfm[dfm$IsRepresent_peaktx=="O",!colnames(dfm) %in% c("readcount", "readcount_link", "libsize", "IsRepresent_peaktx") ] %>% group_by( annot_transcript_id ) %>% dplyr::slice(n=1) 
