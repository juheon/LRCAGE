...
library(tidyr)
options(scipen=999)
#args=c("/bar/jmaeng/pfiles/LRCAGE/test/LRCAGE_WT_DACSB_gtf_talon_read_annot.chr1.tsv", 3)
...
df.no_read_info<-df.no_read_info %>% group_by( dataset, annot_transcript_id  ) %>% dplyr::mutate( readcount=length(annot_transcript_id)) %>% group_by( dataset, annot_transcript_id ) %>% dplyr::slice(n=1)
